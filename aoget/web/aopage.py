...
class AoPage:
    ...
    def get_sorted_filenames_by_extension(self, extension):
        ...
        sorted_stripped_filenames = list(map(lambda url: url.split('/')[-1], sorted_filenames))
        return list(map(lambda filename: urllib.parse.unquote(filename), sorted_stripped_filenames))
